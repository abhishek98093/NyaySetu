...
const ComplaintCard = ({ complaint }) => {
  ...
  // Status configuration - keys now match database values
  const statusConfig = {
    'Pending': {
      bg: 'bg-yellow-50',
    ...
    'In Progress': {  // Changed from 'in-progress'
      bg: 'bg-blue-50',
      text: 'text-blue-800',
    ...
    'Resolved': {  // Changed from 'resolved'
      bg: 'bg-green-50',
      text: 'text-green-800',
    ...
    'Rejected': {  // Changed from 'rejected'
      bg: 'bg-red-50',
      text: 'text-red-800',
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center flex-wrap gap-2">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${status.badge}`}>
                {complaint.crime_type} {/* Changed from complaint.category */}
              </span>
            </div>
          ...
          <div className="flex flex-col items-end gap-1 min-w-[120px]">
            <span className={`px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1 ${status.badge}`}>
              ...
              {normalizedStatus} {/* Display normalized status */}
            </span>
